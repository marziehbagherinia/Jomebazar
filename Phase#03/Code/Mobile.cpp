...
map<string, string> Mobile::getProductDetail()
{
  ...
  informations[CPUF] = cpu;
  informations[BUILTMEM] = to_string(inMemory);
  informations[RAM] = to_string(RaM);
  informations[DISPLAYSIZE] = disp;
  informations[CAMERARES] = to_string(cameraRes);
  informations[OS] = Os;
